...
class Lattice():
    ...
    def events_progressing(self):
        ...
        print(
            'The average covergae is {0} when the maximum simulation time is {1}.'.format(
                average_coverage,
                self.t_max))
    ...
    def plot(self):
        ...
        plt.xlabel('time')
        plt.ylabel('coverage')
        plt.title('direct method (different alpha)')
        plt.savefig('coverage_direct method (different alpha)')
        plt.show()
